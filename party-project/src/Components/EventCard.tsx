...
function EventCard({ event, basePath = "/events", rsvpStatus }: EventCardProps) { // Card component for event details
    ...
    return (
        <Link to={`${basePath}/${event.slug}`}>
            <motion.div className="group grid-rows-2 h-11/12 bg-white hover:cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition duration-300 ease-in-out rounded-xl p-6 m-4 border border-mauve-200 hover:border-pink-500">
