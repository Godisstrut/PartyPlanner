...
if (!authed) return (
    ...
    async function handleSend(e: React.ChangeEvent<HTMLFormElement>) {
        ...
        if (result) {
            setSuccessMsg(`Inbjudan skickad till ${result.email} (${result.groupName})`)
            setEmail("")
            setGroupId("")
            refresh()
    ...
    return (
        <div className="min-h-screen bg-cream-100">
            ...
            <div className="bg-white border-b border-mauve-100 px-8 py-6">
                <p className="font-body text-xs tracking-[0.3em] uppercase text-gold-500 mb-1">Administration</p>
            ...
            <div className="max-w-9/10 mx-auto px-6 py-8 grid grid-cols-1 2xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-1">
                    <div className="bg-white rounded-2xl border border-mauve-100 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6">
                            <div className="flex items-center gap-2 mb-5">
                                ...
                                <h2 className="font-heading text-xl text-mauve-700">Skicka inbjudan</h2>
                            </div>
                            <form onSubmit={handleSend} className="flex flex-col gap-4">
                                <div>
                                    <label className="font-body text-xs tracking-[0.2em] uppercase text-mauve-400 mb-1.5 block">E-postadress</label>
                                    <input
                                        type="email"
                                        value={email}
                                        ...
                                        required
                                        className="w-full font-body text-sm border border-mauve-200 rounded-xl px-4 py-3 text-mauve-700 placeholder:text-mauve-300 bg-cream-50 focus:outline-none focus:border-mauve-400 transition-colors"
                                    />
                                </div>
                                <div>
                                    <label className="font-body text-xs tracking-[0.2em] uppercase text-mauve-400 mb-1.5 block">Grupp</label>
                                    <select
                                        value={groupId}
                                        onChange={(e) => setGroupId(e.target.value)}
                                        required
                                        className="w-full font-body text-sm border border-mauve-200 rounded-xl px-4 py-3 text-mauve-700 bg-cream-50 focus:outline-none focus:border-mauve-400 transition-colors"
                                    >
                                    ...
                                    {groupId && (
                                        <p className="font-body text-xs text-mauve-400 mt-2 pl-1">
                                            Ser: {groups.find((g) => g.id === groupId)?.eventSlugs.join(", ")}
                                        </p>
                                    )}
                                </div>
                                <AnimatePresence>
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 font-body text-sm font-bold tracking-[0.15em] uppercase bg-mauve-700 hover:bg-mauve-800 text-white rounded-xl py-3 px-6 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    <Send size={14} />
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-4">
                        ...
                        <div className="bg-white rounded-xl border border-mauve-100 p-4 text-center">
                            <p className="font-heading text-3xl text-mauve-700">{invites.filter(i => i.openedAt).length}</p>
                        ...
                        {(["events", "invites"] as const).map((tab) => (
                            <button
                                ...
                                className={`font-body text-xs tracking-[0.2em] uppercase px-5 py-2 rounded-lg transition-all duration-200
                                    ${activeTab === tab ? "bg-mauve-700 text-white shadow-sm" : "text-mauve-400 hover:text-mauve-600"}`}
                            >
                                {tab === "events" ? (
                            ...
                            {invitesLoading ? (
                                ...
                                invites.map((invite) => (
                                    <motion.div
                                        ...
                                        className="bg-white rounded-xl border border-mauve-100 px-5 py-4 flex items-center justify-between gap-4"
                                    >
                                        <div className="min-w-0">
                                            <p className="font-body text-sm font-semibold text-mauve-700 truncate">{invite.email}</p>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 font-body text-xs border border-mauve-200 rounded-lg px-3 py-2 text-mauve-500 hover:border-mauve-400 hover:text-mauve-700 transition-colors duration-150"
                                            >
